...
class REPL:
    ...
    def _handle_prompt(self, user_input: str) -> None:
        """
        Handle a natural language prompt - optimized for maximum speed.
        Minimal overhead in the hot path.
        """
        # Get conversation history (limit to 10 for speed)
        conversation_history = self.session.get_recent_history(n=10)
        
        # Build prompt for metrics (reuse llama's format)
        # Cache this to avoid duplicate work, but we need it for metrics
        full_prompt = self.llama.build_prompt(user_input, conversation_history)
        
        # Initialize metrics tracker
        tracker = MetricsTracker()
        tracker.start(full_prompt)
        
        # Add to session (deferred save for speed)
        self.session.add_message("user", user_input)
        
        # Stream response with minimal overhead
        response_content = ""
        ...
        try:
            ...
            # Get stream (this is the hot path - optimize here)
            stream = self.llama.stream_chat(user_input, conversation_history)
            ...
            if first_token:
                ...
                response_content = first_token
                
                # Stop spinner immediately
                spinner.__exit__(None, None, None)
                spinner = None
                
                # Write first token immediately (no buffering)
                sys.stdout.write(first_token)
                sys.stdout.flush()
                
                # Stream remaining tokens with minimal overhead
                # Direct iteration - no function calls in hot path
                for token in stream:
        ...
        # Add response to session (save deferred)
        self.session.add_message("assistant", response_content)
        # Save history now (after response is complete)
        ...
        file_info = self.editor.extract_file_content(response_content)
        if file_info:
        ...
        delete_file = self.editor.extract_delete_file(response_content)
        if delete_file:
